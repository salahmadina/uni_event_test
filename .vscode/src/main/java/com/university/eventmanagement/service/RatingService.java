...
public class RatingService {
    ...
    public String rateEvent(User user, Long eventId, int stars, String comment) {
        ...
        // Only allow rating after the event has taken place (PAST) or been cancelled
        if (event.getStatus() == Event.Status.UPCOMING || event.getStatus() == Event.Status.POSTPONED) {
            return "You can only rate events that have already taken place.";
        }
        ...
        rating.setEventNameSnapshot(event.getName()); // ← snapshot
