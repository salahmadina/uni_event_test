...
public class StudentController {
    ...
    public String myBookings(HttpSession session, Model model) {
        ...
        Map<Long, Boolean> ratedMap    = new HashMap<>();
        Map<Long, Boolean> rateableMap = new HashMap<>(); // can the student rate this event?

        for (Booking booking : bookings) {
            Event event = booking.getEvent();
            if (event != null) {
                boolean rated = ratingService.hasUserRatedEvent(user, event);
                ratedMap.put(event.getId(), rated);
                // Rateable only if event is PAST or CANCELLED and not yet rated
                boolean rateable = !rated &&
                    (event.getStatus() == Event.Status.PAST ||
                     event.getStatus() == Event.Status.CANCELLED);
                rateableMap.put(event.getId(), rateable);
            }
        }
        ...
        model.addAttribute("ratedMap", ratedMap);
        model.addAttribute("rateableMap", rateableMap);
